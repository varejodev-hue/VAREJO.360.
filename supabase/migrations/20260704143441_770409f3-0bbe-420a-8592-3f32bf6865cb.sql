...
BEGIN
  ...
  SELECT
    ...
    (NULLIF(s.codigo_produto,'') IS NOT NULL OR NULLIF(s.descricao_produto,'') IS NOT NULL) AS has_item
  FROM public.orcamentos_staging s
  ...
  -- Itens: só substitui os itens dos pedidos que vieram COM itens nesta importação.
  -- Pedidos sem itens preservam o que já existir em orcamento_itens.
  DELETE FROM public.orcamento_itens oi
  USING public.orcamentos o
  WHERE oi.orcamento_id = o.id
    AND (o.numero, o.loja_id) IN (
      SELECT DISTINCT numero, loja_id FROM _resolved WHERE has_item
    );
