...
CREATE OR REPLACE FUNCTION public.process_orcamentos_staging_chunk(_log_id uuid, _limit int DEFAULT 25000)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Pega um chunk de IDs para processar
  ...
  -- Cadastros: idempotente (WHERE NOT EXISTS) — pode rodar por chunk sem duplicar
  WITH novas AS (
...
REVOKE ALL ON FUNCTION public.process_orcamentos_staging_chunk(uuid, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.process_orcamentos_staging_chunk(uuid, int) TO authenticated, service_role;
