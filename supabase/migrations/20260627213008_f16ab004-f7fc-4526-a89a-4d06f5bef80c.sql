...
CREATE OR REPLACE FUNCTION public.carteira_por_loja(
  p_inicio date DEFAULT (CURRENT_DATE - INTERVAL '180 days')::date,
  p_fim date DEFAULT CURRENT_DATE
)
RETURNS TABLE (
  loja_id uuid, loja_nome text, canal text,
  qtd_vendedores integer,
  qtd_especificadores integer, qtd_ativos integer, qtd_risco integer, qtd_inativos integer,
  valor_orcado numeric, valor_vendido numeric,
  conversao_pct numeric, ticket_medio numeric, ultimo_contato date
)
...
BEGIN
  ...
  SELECT
    l.id, l.nome, l.canal,
    COALESCE(a.qtd_v,0),
    COALESCE(a.qtd,0), COALESCE(a.qtd_a,0), COALESCE(a.qtd_r,0), COALESCE(a.qtd_i,0),
...
REVOKE ALL ON FUNCTION public.carteira_por_loja(date, date) FROM public;
GRANT EXECUTE ON FUNCTION public.carteira_por_loja(date, date) TO authenticated, service_role;
