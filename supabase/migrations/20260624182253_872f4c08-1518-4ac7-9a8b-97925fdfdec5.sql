DROP FUNCTION IF EXISTS public.turnover_especificadores_migracao(date, date, uuid);
...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT o.especificador_id AS esp_id, o.loja_id AS lj_id, o.vendedor_id AS vd_id,
  ...
  loja_rank AS (
    SELECT b.esp_id, b.metade, b.lj_id,
      SUM(b.valor) AS w,
      ROW_NUMBER() OVER (PARTITION BY b.esp_id, b.metade ORDER BY SUM(b.valor) DESC) AS rn
    FROM base b WHERE b.lj_id IS NOT NULL
    GROUP BY b.esp_id, b.metade, b.lj_id
  ),
  vend_rank AS (
    SELECT b.esp_id, b.metade, b.vd_id,
      SUM(b.valor) AS w,
      ROW_NUMBER() OVER (PARTITION BY b.esp_id, b.metade ORDER BY SUM(b.valor) DESC) AS rn
    FROM base b WHERE b.vd_id IS NOT NULL
  ...
  tot AS (
    ...
    FROM base b GROUP BY b.esp_id
  )
  SELECT
    e.id, e.nome,
    lo.lj_id, ll_o.nome, ll_o.canal::text,
    la.lj_id, ll_a.nome, ll_a.canal::text,
    vo.vd_id, vno.nome,
    va.vd_id, vna.nome,
  ...
  LEFT JOIN loja_rank lo ON lo.esp_id = t.esp_id AND lo.metade = 'antes' AND lo.rn = 1
  LEFT JOIN loja_rank la ON la.esp_id = t.esp_id AND la.metade = 'depois' AND la.rn = 1
  LEFT JOIN vend_rank vo ON vo.esp_id = t.esp_id AND vo.metade = 'antes' AND vo.rn = 1
  LEFT JOIN vend_rank va ON va.esp_id = t.esp_id AND va.metade = 'depois' AND va.rn = 1
