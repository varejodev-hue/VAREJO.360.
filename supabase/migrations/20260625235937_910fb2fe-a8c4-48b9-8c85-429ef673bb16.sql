...
ALTER TABLE public.orcamentos RENAME COLUMN linha TO linha_produto;
ALTER TABLE public.orcamentos_staging ADD COLUMN IF NOT EXISTS linha_produto text;

DROP INDEX IF EXISTS public.idx_orcamentos_loja_linha;
CREATE INDEX IF NOT EXISTS idx_orcamentos_loja_linha_produto ON public.orcamentos(loja_id, linha_produto);
...
BEGIN
  ...
  FROM public.orcamentos_staging s
  WHERE s.log_id = _log_id AND s.loja IS NOT NULL AND s.loja <> ''
  ...
  WITH resolved AS (
    ...
    WHERE s.log_id = _log_id AND s.numero IS NOT NULL AND s.numero <> ''
  ...
  UPDATE public.import_logs SET
    ...
    cadastros_criados = jsonb_build_object(
      'lojas', v_lojas_criadas, 'vendedores', v_vend_criados,
      'especificadores', v_esp_criados, 'clientes', v_cli_criados,
      'lojas_sem_canal', v_lojas_sem_canal,
      'inseridos', v_inseridos, 'atualizados', v_atualizados
    )
  WHERE id = _log_id;

  RETURN jsonb_build_object(
    'sucesso', v_sucesso, 'inseridos', v_inseridos, 'atualizados', v_atualizados,
    ...
    'lojas_sem_canal', v_lojas_sem_canal, 'total_staging', v_total
  );
END;
$function$;
...
RETURNS TABLE(
  loja_id uuid, loja_nome text, cobertura_pct numeric,
  top_categorias jsonb, top_linhas jsonb, top_formatos jsonb
)
...
BEGIN
  ...
  WITH base AS (
    SELECT o.loja_id,
      ...
      NULLIF(trim(o.linha_produto),'') AS linha_produto,
  ...
  cobertura AS (
    SELECT loja_id, COUNT(*) AS total,
      COUNT(*) FILTER (WHERE categoria IS NOT NULL OR linha_produto IS NOT NULL OR formato IS NOT NULL) AS preench
  ...
  cat AS (
    ...
    FROM base WHERE categoria IS NOT NULL GROUP BY loja_id, categoria
  ),
  lin AS (
    SELECT loja_id, linha_produto AS nome, SUM(valor) AS v, COUNT(*) AS q,
      ROW_NUMBER() OVER (PARTITION BY loja_id ORDER BY SUM(valor) DESC) AS rn
    FROM base WHERE linha_produto IS NOT NULL GROUP BY loja_id, linha_produto
  ),
  fmt AS (
    SELECT loja_id, formato AS nome, SUM(valor) AS v, COUNT(*) AS q,
      ROW_NUMBER() OVER (PARTITION BY loja_id ORDER BY SUM(valor) DESC) AS rn
    FROM base WHERE formato IS NOT NULL GROUP BY loja_id, formato
  ),
  cat_j AS (SELECT loja_id, jsonb_agg(jsonb_build_object('nome',nome,'valor',v,'qtd',q) ORDER BY rn) AS items FROM cat WHERE rn<=5 GROUP BY loja_id),
  lin_j AS (SELECT loja_id, jsonb_agg(jsonb_build_object('nome',nome,'valor',v,'qtd',q) ORDER BY rn) AS items FROM lin WHERE rn<=5 GROUP BY loja_id),
  fmt_j AS (SELECT loja_id, jsonb_agg(jsonb_build_object('nome',nome,'valor',v,'qtd',q) ORDER BY rn) AS items FROM fmt WHERE rn<=5 GROUP BY loja_id)
  SELECT l.id, l.nome,
    CASE WHEN COALESCE(c.total,0)>0 THEN ROUND((c.preench::numeric/c.total)*100,2) ELSE 0 END,
    COALESCE(cj.items,'[]'::jsonb), COALESCE(lj.items,'[]'::jsonb), COALESCE(fj.items,'[]'::jsonb)
  FROM public.lojas l
